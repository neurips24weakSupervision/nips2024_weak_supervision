...
def quantization_consistency_loss(original, quantized):
#    quantized = tf.stop_gradient(quantized)  # Stop gradients for quantized vectors
    return tf.reduce_mean(tf.square(original - quantized))
...
def calcSP(attn,resolution,batch_size):
    ...
    grid =  tf.broadcast_to(grid, (batch_size,11,resolution[0],resolution[1],4))
    attn = tf.reshape(attn,(batch_size,11,resolution[0],resolution[1]))
    attn = tf.expand_dims(attn, axis= - 1)
    gridRel = tf.multiply(grid, attn)
    weighted_sum_p = tf.reduce_sum(gridRel, axis=[2, 3]) / (tf.reduce_sum(attn, axis=[2, 3])+0.000000001)
    return weighted_sum_p[:, :, :2]
...
def var_loss(masks):
  means = calcSP(masks,(128,128),64)
  transposed_means = tf.transpose(means, perm=[1, 0, 2])
  stacked_x, stacked_y = tf.split(transposed_means, num_or_size_splits=2, axis=2)
  mean_x = tf.reduce_mean(stacked_x, axis=1, keepdims=True)
  mean_y = tf.reduce_mean(stacked_y, axis=1, keepdims=True)
  variance_x = tf.reduce_mean(tf.square(stacked_x - mean_x), axis=1)
  variance_y = tf.reduce_mean(tf.square(stacked_y - mean_y), axis=1)
  flat_variance_x = tf.reshape(variance_x, [-1])
  flat_variance_y = tf.reshape(variance_y, [-1])
  all_variances = tf.concat([flat_variance_x, flat_variance_y], axis=0)
  total_variance_sum = tf.reduce_sum(tf.math.reciprocal(all_variances+0.0000001))

  return total_variance_sum, (flat_variance_x), (means)
